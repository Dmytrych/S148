    ...
    async function fetchData() {
      ...
      cart.forEach((cartProduct) => {
        const retrievedProduct = retrievedProducts.find(
          (retrievedProduct) => retrievedProduct.id === cartProduct.productId,
        );
        if (!retrievedProduct) {
      ...
      if (
        failedToFindCartProducts
                && failedToFindCartProducts.length > 0
      ) {
        removeProductsFromCart(failedToFindCartProducts);
      }
      if (
        successfullyFoundProducts
                && successfullyFoundProducts.length > 0
      ) {
