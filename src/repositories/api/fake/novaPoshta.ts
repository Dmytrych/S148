...
export async function getCities(filter) {
  return Promise.resolve(
    fakeCities.filter((city) => city.description.includes(filter)),
  );
}

// @ts-ignore
export async function getWarehouseByNumber(cityName, cityRef, warehouseNumber) {
  const warehouse = fakeWarehouses.find(
    (fakeWarehouse) => fakeWarehouse.warehouseNumber.toString() === warehouseNumber,
  );
