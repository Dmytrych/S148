...
const ProductDetails = ({ productId }: IProductDetailsParams) => {
  ...
  useEffect(() => {
    const defaultProductProperties = product?.options.map(
      (option) => ({
        option,
  ...
  function setOptionValue(
    ...
    const changedProperty = productProperties.find(
      (property) => property.option.name === optionName,
    );
    if (!changedProperty){
      return;
    }
