...
} from '../../repositories/api/Dto/OptionDto';
...
export const ProductVersions = ({
  ...
  onChange,
  defaultValue,
}: ProductVersionsDisplayInput) => {
  const [selection, setSelection] = useState(
    defaultValue ?? option.variants[0],
  );
                    ...
                    <div key={variant.name}>
                        {option.type !== OptionType.Image ? (
                            <TextProductVariant
                                ...
                                selected={variant.name === selection.name}
...
const ImageVariant = ({ variant, selected, onClick }: ImageVariantInput) => (
    <ImageVariantContainer selected={selected} onClick={onClick}>
